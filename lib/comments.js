...
function mapRow(r) {
  return {
    ...
    // SPA-510 P0-1：行内落库优先，无落库（旧数据）读时 join users 回退
    authorAvatarHash: r.author_avatar_hash ?? r.joined_avatar_hash ?? null,
    body: r.body,
...
export function getComment(db, id) {
  const r = db
    .prepare(
      `SELECT c.*, u.avatar_hash AS joined_avatar_hash FROM event_comments c
       LEFT JOIN users u ON u.discord_id = c.author_discord_id
       WHERE c.id = ?`,
    )
    .get(id);
...
export function listComments(db, eventId) {
  return db
    .prepare(
      `SELECT c.*, u.avatar_hash AS joined_avatar_hash FROM event_comments c
       LEFT JOIN users u ON u.discord_id = c.author_discord_id
       WHERE c.event_id = ? ORDER BY c.created_at ASC, c.id ASC`,
    )
    .all(eventId)
    .map(mapRow);
}
 ...
 */
export function createComment(db, { eventId, parentId = null, authorDiscordId, authorUsername, authorAvatarHash = null, body, requestId = null }) {
  ...
  try {
    const out = db
      .prepare(
        "INSERT INTO event_comments (event_id, parent_id, author_discord_id, author_username, author_avatar_hash, body, request_id, created_at) VALUES (?,?,?,?,?,?,?,?)",
      )
      .run(eventId, l1Id, authorDiscordId, authorUsername, authorAvatarHash ?? null, String(body).trim(), requestId, now);
...
// SPA-513：回复入口与编辑/删除解耦——登录且非终态可读时人人可回复，
// 编辑仅作者、删除仅作者+房主。返回三者标志，皆无时调用方渲染 null。
export function visibleCommentActions({ comment, meId, creatorId, terminal }) {
  const reply = !!meId && !terminal;
  const mine = !!meId && comment.authorDiscordId === meId;
  const del = canDeleteComment({ comment, meId, creatorId });
  return { reply, edit: mine, del };
}
