// Components V2 卡片 payload 组装（design-spec §7；accent 语义 §5，解耦 §6）。
...
export function buildCardPayload(event, siteUrl) {
  ...
  if (isFull({ confirmed: event.confirmed, held: event.held ?? 0, cap: event.cap }) && event.waitlisted > 0) {
    components.push({ type: 10, content: `⚠️ 已满员，新报名将进入排队（排队 ${event.waitlisted}）` });
  }
  ...
  return { flags: COMPONENTS_V2_FLAG, accent, components };
}
