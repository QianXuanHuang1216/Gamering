...
export async function precheckChannel(channelId) {
  ...
  return { ok: true, channel: data, transport };
}

/**
 * 发卡。显式带 nonce 时加 enforce_nonce：Discord 在「最近几分钟」内按 (作者, nonce) 去重，
 * 同 nonce 重发拿回原来那张而不是多一张——「到底发没发出去」那个窗口里，这能从源头兜住。
 * Components V2 的限制只针对 content/embeds/sticker_ids/poll，nonce/enforce_nonce 是顶层
 * 字段，flags 带不带 1<<15 都不影响它们。没带 nonce 就别声明去重：装一副防过了的样子最糟。
 */
export async function sendCard(channelId, payload, { nonce } = {}) {
  const body = nonce == null ? payload : { ...payload, nonce, enforce_nonce: true };
  return discordApi(`/channels/${channelId}/messages`, { method: "POST", body });
}
