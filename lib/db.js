...
/** 记一张活卡。同一张 Discord 消息只记一次：enforce_nonce 会让重发拿回原来那张，
 *  再插一行会让 liveMessages 数出两张卡（fanout 也会对着同一张 PATCH 两遍）。 */
export function addMessage(db, { eventId, guildId, channelId, messageId }) {
  db.prepare(
    `INSERT INTO event_messages (event_id, guild_id, channel_id, message_id, created_at)
     SELECT ?, ?, ?, ?, ?
     WHERE NOT EXISTS (SELECT 1 FROM event_messages WHERE event_id = ? AND message_id = ?)`,
  ).run(eventId, guildId, channelId, messageId, Date.now(), eventId, messageId);
}
...
export function latestLiveCard(db, eventId, channelId) {
  const rows = liveMessages(db, eventId).filter((r) => r.channel_id === channelId);
