...
export function enqueueRetry(db, { eventId, channelId, messageId, status }) {
  const nextDue = Date.now() + 60_000;
  const existing = db
    .prepare("SELECT * FROM sync_retries WHERE event_id = ? AND message_id = ?")
    .get(eventId, messageId);
  if (existing) {
    db.prepare("UPDATE sync_retries SET attempts = attempts + 1, next_due = ?, last_status = ? WHERE id = ?").run(
      nextDue * Math.min(existing.attempts + 1, 8),
      status ?? null,
      existing.id,
    );
    return;
  ...
  ).run(eventId, channelId, messageId, nextDue, status ?? null, Date.now());
}
